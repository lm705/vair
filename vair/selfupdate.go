...
// raw.githubusercontent.com is often the blocked one. The author publishes
// version.json (and the binary it points to) on each release.
...
type updateManifest struct {
	Version string `json:"version"`         // e.g. "1.9.0"
	URL     string `json:"url"`             // download URL of the new exe
	SHA256  string `json:"sha256"`          // hex SHA-256 of the exe (mandatory)
	Notes   string `json:"notes,omitempty"` // short "what's new"
}

// updateInfo is what /api/update/check returns to the UI.
...
func parseSemver(s string) [3]int {
	...
	for i, p := range strings.SplitN(s, ".", 3) {
		// stop at any non-numeric suffix (e.g. "1.9.0-rc1")
		num := p
		for j := 0; j < len(p); j++ {
			if p[j] < '0' || p[j] > '9' {
				num = p[:j]
...
// connected (proxy mode → the local HTTP proxy; TUN mode → already system-wide),
// so the check/download still works when GitHub is blocked on the bare link.
...
// replace-and-relaunch. Progress is streamed via SSE. Caller should run this in
// a goroutine. The SHA check is mandatory: a manifest without one is rejected.
func runUpdate() {
	if updateRunning {
		return
	}
