...
func memSig(q windowQuery, favorites []string) string {
	return q.sort + "\x00" + q.filter + "\x00" + strings.Join(q.proto, ",") + "\x00" +
...
func memOrder(tabID string, q windowQuery, favorites []string) []*ConfigEntry {
	...
	if (q.sort == "" || q.sort == "idx") && q.filter == "" && len(q.proto) == 0 && !q.dedupHide && len(favorites) == 0 {
		state.mu.RLock()
...
func speedRank(k *sortKey) int {
	if k.sStatus == StatusOK && k.speed > 0 {
		return 0
	}
...
// then any tab. Returns the entry and the tab it was found in (Raw is effectively
// immutable, so an unlocked read is safe enough for this infrequent path).
